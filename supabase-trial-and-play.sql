-- ============================================================================
-- Effyra – Trial (100 Credits / 7 Tage) serverseitig + Google-Play-Entitlements
-- ----------------------------------------------------------------------------
...
--      • Free innerhalb der 7-Tage-Testphase → 100 Credits (+ nachgekaufte ai_extra)
...
begin
  ...
  -- ================= FREE-TESTPHASE (100 Credits / 7 Tage) + gekaufte Credits =================
  select * into p from public.profiles where id = p_user for update;
  if not found then return json_build_object('ok', false, 'reason', 'no_profile'); end if;
  if p.usage_month is distinct from cur_month then p.usage_month := cur_month; p.ai_used := 0; end if;   -- ai_extra bleibt!
